...
! -------------------------------------------------------------------------!
  ...
  do it = 1,iter
    ...
    ! RED DOTS (Even coordinte sums)
    ! boundary:
    call RB_Eval_bound(u,delta,coords,N,N_loc,0)
    ...
    ! BLACK DOTS (Uneven coordinate sums)
    ! boundary:
    call RB_Eval_bound(u,delta,coords,N,N_loc,1)
...
! ------------------------------------------------------------------------ !
  ...
  real(dp), intent(inout), dimension(:,:,:) :: u
  integer,  intent(in) :: coords(3),N,N_loc
  real(dp), intent(in) :: delta
  integer,  intent(in) :: col ! 0 for red and 1 for black
  ...
  do kk = 1,2
    k = bounds(kk)
    do j = 2,N_loc-1
      do i = 2+mod(k+j+col,2),N_loc-1,2
        u(i,j,k) =(u(i-1,j,k)+u(i+1,j,k)&!X-direction
                  +u(i,j-1,k)+u(i,j+1,k)&!Y-direction
                  +u(i,j,k-1)+u(i,j,k+1)&!Z-direction
                  +delta**2._dp*evalRadiator(i,j,k,N_loc,coords,N)&
  ...
  !$OMP do schedule(static)
    ...
    do ii = 1,2
	  i = bounds(ii)
      do j = 2+mod(k+i+col,2),N_loc-1,2
        u(i,j,k) =(u(i-1,j,k)+u(i+1,j,k)&!X-direction
                  +u(i,j-1,k)+u(i,j+1,k)&!Y-direction
                  +u(i,j,k-1)+u(i,j,k+1)&!Z-direction
                  +delta**2._dp*evalRadiator(i,j,k,N_loc,coords,N)&
...
program main
  ...
  ! MPI Variables
  integer :: isize,irank,ierr,errc, iprovided
  ...
  !CALL MPI_Init(ierr)
  CALL MPI_init_thread(MPI_THREAD_FUNNELED,iprovided,ierr)
!  CALL MPI_init_thread(MPI_THREAD_SERIALIZED,iprovided,ierr)
